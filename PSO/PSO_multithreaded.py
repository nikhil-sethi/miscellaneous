# multithreaded particle swarm implementation
...
GMax = 20  # max PSO iterations
x_bound = 1000  # Max geofence abscissa
y_bound = 1000  # Max geofence ordinate
...
class MyPlotClass:
    ...
    def run(self, i):
        ...
        self.hLine.set_data(current_pos[0], current_pos[1])
...
class ThreadClass(threading.Thread):
    ...
    def run(self, lastime=time.time()):
        ...
        for G in range(1, GMax + 1):
            deltaD = w * deltaD + c1*random.random() * (self._particleClass.best.current_pos - current_pos) + c2*random.random() * (self._globalClass.current_pos - current_pos)
            ...
            #print(current_pos)
            # print("updating data")
            print('target=%s for %s ' % (target_pos, self.n))
            checktime = time.time()
            while distance.euclidean(current_pos, target_pos) > crit_dist:
                ...
                if lastime-checktime > 1:
                    print('current_postion=%s for %s' % (current_pos, self.getName()))     #print current_postion every five seconds
                    checktime = time.time()
                ...
                if self._particleClass.fitness > self._particleClass.best.fitness:
                    ...
                    if self._particleClass.best.fitness > self._globalClass.fitness:
                        ...
                        print('Globalbest ', self._globalClass.current_pos, self.getName())
...
for n in range(1, NMax + 1):
    ...
    particle[n].best = ParticleClass(particle[n].current_pos)  # create a personal best object as an attribute of the respective particle
...
#start threads and plot classes
for n in range(1, NMax + 1):
...
#                                 c1 * random.random() * (particles[n].best.current_postion - particles[n].current_postion) + \
#                                 c2 * random.random() * (GlobalBest.current_postion - particles[n].current_postion)
#         particles[n].current_postion = particles[n].current_postion + particles[n].velocity
#
#
#         #for i, human_pos in enumerate(humans.locations):
#          #   particles[n].fitness = ObjFun(particles[n].current_postion, human_pos)  # calculate new fitness at new current_postion
#         particles[n].fitness = testFun(particles[n].current_postion)
...
#             particles[n].best.current_postion = particles[n].current_postion
#
...
#
#         plt.plot(particles[n].current_postion[0], particles[n].current_postion[1], 'o', color='b')
#         plt.annotate(n, (particles[n].current_postion[0], particles[n].current_postion[1]))
